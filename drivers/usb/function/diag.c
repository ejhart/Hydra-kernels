...
int diag_check_online(struct diag_context *ctxt)
{
	if (!ctxt->online) {
		DBG("diag: diag_read: no usb cable , ctxt->error %d \n",ctxt->error);
		ctxt->isRead = 0;
			...
			size_t count, loff_t *pos)
{
	...
	if(diag_check_online(ctxt) == -1){
		return -1;
	}

	/* we will block until we're online */
	while (!(ctxt->online || ctxt->error)) {
		printk(KERN_INFO "diag:diag_read: waiting for online state, ctxt->error %d, online %d \n",
			ctxt->error,ctxt->online);
			 ...
			 size_t count, loff_t *pos)
{
	...
	while (count > 0) {
		...
		req = 0;
		if (!ctxt->online) {
			DBG("diag: diag_write: no usb cable 1\n");
			r= -1;
			break;
		}
		...
		if (!ctxt->online) {
			DBG("diag: diag_write: no usb cable 2\n");
			r= -1;
			break;
		}
		if (req != 0) {
	  ...
	  unsigned int cmd, unsigned long arg)
{
	...
	case USB_DIAG_FUNC_IOC_ENABLE_SET:
		...
		if (tmp_value)	{
			...
			printk(KERN_INFO "%s:  enable diag, online %d", __func__,ctxt->online);
			usb_function_enable(DIAG_FUNCTION_NAME, 1);
			smd_diag_enable("diag_ioctl", 1);
		}	else	{
			smd_diag_enable("diag_ioctl", 0);
			usb_function_enable(DIAG_FUNCTION_NAME, 0);
			printk(KERN_INFO "%s: disable diag, online %d", __func__,ctxt->online);
...
static int diag_open(struct inode *ip, struct file *fp)
{
	...
	DBG("diag:diag:diag_open()\n");

	if (_lock(&ctxt->open_excl)){
		DBG("diag:diag:diag_open() fail\n");
		return -EBUSY;
	}
			 ...
			 size_t count, loff_t *pos)
{
	...
	if (var == 0x1 || var == 0x31) {
		...
		smd_diag_enable("diag_enable_write", 1);
		...
		usb_function_enable(DIAG_FUNCTION_NAME, 0);
		smd_diag_enable("diag_enable_write", 0);
	}
...
static int msm_diag_probe(struct platform_device *pdev)
{
	...
	/*open SMD_DIAG if radio flag is set to 8 20000*/
	if (pdev->id == 0 && !usb_func_diag.disabled)
		smd_diag_enable("msm_diag_probe", 1);
...
static int diag_set_enabled(const char *val, struct kernel_param *kp)
{
	...
	if (enabled)
		smd_diag_enable("diag_set_enabled", 1);
	else
		smd_diag_enable("diag_set_enabled", 0);
