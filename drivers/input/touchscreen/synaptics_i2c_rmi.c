					...
					const char *buf, size_t count)
{
	...
	if (pt_count >= 4) {
		...
		if (val[1] < 0 || val[1] > ts->display_width) { /* Right */
			...
			ts->margin_inactive_raw[1] = ts->ts_raw_pos[1] - val[1] *
							ts->ts_raw_width / ts->display_width;
		}

		if (val[2] < 0 || val[2] > ts->display_height) { /* top */
			ts->margin_inactive_raw[2] = ts->margin_inactive_pixel[2] = 0;
		} else {
			ts->margin_inactive_pixel[2] = val[2];
			ts->margin_inactive_raw[2] = ts->ts_raw_pos[2] + val[2] *
							ts->ts_raw_height / ts->display_height;
		}

		if (val[3] < 0 || val[3] > ts->display_height) { /* Bottom */
			ts->margin_inactive_pixel[3] = 0;
			ts->margin_inactive_raw[3] = ts->max[1];
		} else {
			ts->margin_inactive_pixel[3] = val[3];
			ts->margin_inactive_raw[3] = ts->ts_raw_pos[3] - val[3] *
							ts->ts_raw_height / ts->display_height;
		}
	}
...
static DEVICE_ATTR(margin_area, 0666, margin_area_show, margin_area_store);

static int margin_filter(struct synaptics_ts_data *ts, int pos[2][2],
						const int finger2_pressed, const int z)
{
	uint8_t finger_result = 0;

	if ((ts->last_finger_result == 1 || ts->last_finger_result == 2)
			&& ts->last_finger2_pressed && !finger2_pressed) {
		/* Finger from two becomes one, re-enable local margin filter */
		ts->disable_margin_filter[0] = ts->disable_margin_filter[1] = 0;
	}
	ts->last_finger2_pressed = finger2_pressed;

	if ((pos[0][0] > ts->margin_inactive_raw[0] &&
		pos[0][0] < ts->margin_inactive_raw[1] &&
		pos[0][1] > ts->margin_inactive_raw[2] &&
		pos[0][1] < ts->margin_inactive_raw[3]) ||
		ts->disable_margin_filter[0]) {
		finger_result |= FINGER_1_REPORT;
		ts->disable_margin_filter[0] = 1;
	}
	if (finger2_pressed) {
		if ((pos[1][0] > ts->margin_inactive_raw[0] &&
			pos[1][0] < ts->margin_inactive_raw[1] &&
			pos[1][1] > ts->margin_inactive_raw[2] &&
			pos[1][1] < ts->margin_inactive_raw[3]) ||
			ts->disable_margin_filter[1]) {
			finger_result |= FINGER_2_REPORT;
			ts->disable_margin_filter[1] = 1;
		}
	}
		...
		if (ret < 0) {
			...
			if ((buf[buf_len - 1] & 1) == 0) {
				...
				if (ts->margin_inactive_pixel[0] || ts->margin_inactive_pixel[1] ||
					...
					ret = margin_filter(ts, pos, finger2_pressed, z);
					if (ret == 1) {
						/* FINGER_1_REPORT */
						finger2_pressed = 0;
						ts->last_finger_result = FINGER_1_REPORT;
						ts->disable_margin_filter[1] = 0;
					} else if (ret == 2) {
						/* FINGER_2_REPORT */
						/* Replace finger 1 to finger 2 */
						finger2_pressed = 0;
						pos[0][0] = pos[1][0];
						pos[0][1] = pos[1][1];
						ts->last_finger_result = FINGER_2_REPORT;
						ts->disable_margin_filter[0] = 0;
					} else if (ret == 0) {
						if (ts->last_finger_result) {
							z = 0;
							ts->last_finger_result = 0;
						} else {
							/* printk("%s: margin_filter\n", __func__); */
							break;
						}
					} else {
						ts->last_finger_result = FINGER_1_REPORT | FINGER_2_REPORT;
					}
				}
				...
				} else if (ts->key_pressed[0] && !finger) {
					ts->key_pressed[0] = 0;
					printk(KERN_INFO "E1@%d, %d\n", pos[0][0], pos[0][1]);
				}
				if (!ts->key_pressed[1] && finger2_pressed) {
					ts->key_pressed[1] = 1;
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts->margin_inactive_raw[0] = ts->ts_raw_pos[0] + ts->margin_inactive_pixel[0] *
						ts->ts_raw_width / ts->display_width; /* Left */
	ts->margin_inactive_raw[1] = ts->ts_raw_pos[1] - ts->margin_inactive_pixel[1] *
						ts->ts_raw_width / ts->display_width; /* Right */
	ts->margin_inactive_raw[2] = ts->ts_raw_pos[2] + ts->margin_inactive_pixel[2] *
						ts->ts_raw_height / ts->display_height; /* Top */
	ts->margin_inactive_raw[3] = ts->ts_raw_pos[3] - ts->margin_inactive_pixel[3] *
						ts->ts_raw_height / ts->display_height; /* Bottom */
